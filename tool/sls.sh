...
function GetLogStore() {
    ...
    aliyunlog log get_logtail_config --project_name="${project}" --config_name="${logtail}" \
    ...
    Info "get logtail ${project} ${logstore} success" || {
      rm -rf "${prefix}/${logstore}.logtail.json"
      Info "get logtail ${project} ${logstore} empty"
    }
}
...
function GetMachineGroup() {
    ...
    local endpoint="$3"
    local project="$4"
    local machine_group="$5"
    local prefix="$6"
    mkdir -p "${prefix}"

    machine_group_count=$(
      aliyunlog log list_machine_group --access-id="${ak}" --access-key="${sk}" --region-endpoint="${endpoint}" --project_name="${project}" | \
        jq -r ".count"
    )
    if [ -z "${machine_group_count}" ]; then
      Warn "machine_group ${machine_group} not found"
      return 1
    fi

    aliyunlog log get_machine_group --project_name="${project}" --group_name="${machine_group}" \
      --access-id="${ak}" --access-key="${sk}" --region-endpoint="${endpoint}" | jq . >"${prefix}/${machine_group}.machine_group.json"; test "${PIPESTATUS[0]}" -eq 0 &&
    Info "get machine_group ${project} ${machine_group} success" ||
    Warn "get machine_group ${project} ${machine_group} failed"
}

function PutMachineGroup() {
    local ak="$1"
    local sk="$2"
    local endpoint="$3"
    local project="$4"
    ...
    if [[ ! -e "${prefix}/${machine_group}.machine_group.json" ]]; then
        return 0
    fi

    aliyunlog log get_machine_group --project_name="${project}" --group_name="${machine_group}" --access-id="${ak}" --access-key="${sk}" --region-endpoint="${endpoint}" && {
        aliyunlog log update_machine_group --access-id="${ak}" --access-key="${sk}" --region-endpoint="${endpoint}" --project_name="${project}" \
            --group_detail="file://./${prefix}/${machine_group}.machine_group.json" &&
        Info "update group_detail ${project} ${logstore} success" ||
        Warn "update group_detail ${project} ${logstore} failed"
    } || {
        aliyunlog log create_machine_group --access-id="${ak}" --access-key="${sk}" --region-endpoint="${endpoint}" --project_name="${project}" \
            --group_detail="file://./${prefix}/${machine_group}.machine_group.json" &&
        Info "create group_detail ${project} ${machine_group} success" ||
        Warn "create group_detail ${project} ${machine_group} failed"
    }
}
...
function PutLogStore() {
    ...
    if [[ ! -e "${prefix}/${logstore}.logtail.json" ]]; then
        return 0
    fi
...
function main() {
    ...
    case "${action}" in
        ...
        "GetAlert") GetAlert "$@";;
        "GetMachineGroup") GetMachineGroup "$@";;
        "PutMachineGroup") PutMachineGroup "$@";;
