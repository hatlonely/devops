...
function KillZombiePID() {
    local namespace=$1
    local pod_name=$2
    local process=$3
    kubectl exec "${pod_name}" -n "${namespace}" -- /bin/sh -ec "ps -e -o pid,ppid,cmd,%mem,etimes,%cpu --sort=-%cpu | head -20 | grep \"$process ver:\" | awk '{
        pid=\$1
        time=\$6
        cpu=\$7
...
function Help() {
    ...
    echo "  sh monitor.sh find_zombie_pid webword-20210929 webword"
    echo "  sh monitor.sh kill_zombie_pid webword-20210929 webword"
}
    ...
    kubectl get pods -A -o wide | grep "${pod_filter}" | while read -r line; do
        ...
        case ${action} in
            ...
            "kill_zombie_pid")
                local process=$3
                printf "%s\t%s\t%s\n" "${nodeip}" "${pod_name}" "${etime}"
                KillZombiePID "${namespace}" "${pod_name}" "${process}"
                ;;
            "find_zombie_pid")
                local process=$3
                printf "%s\t%s\t%s\n" "${nodeip}" "${pod_name}" "${etime}"
                FindZombiePID "${namespace}" "${pod_name}" "${process}"
                ;;
