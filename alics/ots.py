...
def list_table(client: tablestore.OTSClient):
    res = client.list_table()
    return json.loads(json.dumps(res))


def create_table(client: tablestore.OTSClient, table, meta):
...
def describe_table(client: tablestore.OTSClient, table):
...
def get_range(client: tablestore.OTSClient, table):
...
def put_row(client: tablestore.OTSClient, table, row, change=False):
    if not change:
...
def update_row(client: tablestore.OTSClient, table, row, change=False, view=True):
...
def get_row(client: tablestore.OTSClient, table, primary_key):
    meta = client.describe_table(table)
    keys = [v[0] for v in meta.table_meta.schema_of_primary_key]
    pks = []
    for key in keys:
...
def del_row(client: tablestore.OTSClient, table, primary_key):
    meta = client.describe_table(table)
    keys = [v[0] for v in meta.table_meta.schema_of_primary_key]
    pks = []
    for key in keys:
        pks.append((key, primary_key[key]))
    client.delete_row(table, tablestore.Row(pks), None)
    return True


def get_canary_deploy(client: tablestore.OTSClient, table, primary_key, column):
    row = get_row(client, table, primary_key)
    canary_deploys = row["Columns"][column]
    return json.loads(canary_deploys)


def put_canary_deploy(client: tablestore.OTSClient, table, primary_key, column, deploy, change=False):
...
def del_canary_deploy(client: tablestore.OTSClient, table, primary_key, column, deploy, change=False):
    old_deploys = get_canary_deploy(client, table, primary_key, column)
    new_deploys = []
    for old_deploy in old_deploys:
        if all([old_deploy[k] == deploy[k] for k in ["ObjectType", "ObjectId", "Provider", "Function"]]):
...
def get_task(client: tablestore.OTSClient, table, owner_id, project, task_id):
...
def get_configcenter(client: tablestore.OTSClient, kms_client, table):
    res = {}
...
def get_configcenter_key(client: tablestore.OTSClient, kms_client, table, pk):
    vs = pk.split("_")
    block = vs[0]
    section = vs[1]
    key = vs[2]
...
def put_configcenter(client: tablestore.OTSClient, table, config, change, kms_client, cmkey, encrypt):
    for pk in config:
        put_configcenter_key(client, table, pk, config[pk], change, kms_client, cmkey, encrypt)


def put_configcenter_key(client: tablestore.OTSClient, table, pk, val, change, kms_client, cmkey, encrypt):
    vs = pk.split("_")
    block = vs[0]
    section = vs[1]
    key = vs[2]
...
""")
    ...
    parser.add_argument("-a", "--action", help="action", choices=[
        "ListTable", "GetRange", "PutRow", "UpdateRow", "GetRow", "DelRow", "CreateTable", "DescribeTable",
    ...
    if args.action == "ListTable":
        ...
        if row is not None:
            print(json.dumps(row))
    elif args.action == "DelRow":
        print(json.dumps(del_row(client, args.table, json.loads(args.primary_keys))))
