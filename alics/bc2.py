...
def create_cluster(client: bcsc.Client, cluster_desc_str):
    req = bcsm.CreateClusterRequest().from_map(json.loads(cluster_desc_str))
    res = get_cluster(client, req.project, None, req.name)
    if res:
        return res
...
def update_cluster2(client: bcsc.Client, cluster_desc_str, cluster_name, change=False):
    req = bcsm.UpdateClusterRequest().from_map(cluster_desc_str)
    if not req.cluster_id:
        if not cluster_name:
            raise Exception("cluster_name or cluster_id is required")
        res = get_cluster(client, req.project, None, req.name)
        if not res:
            raise Exception("cluster [{}] not exists")
        req.cluster_id = res.to_map()

    if not change:
        res = get_cluster(bcsc.Client, req.project, req.cluster_id, cluster_name)
        diff.color_diff(res, json.loads(cluster_desc_str))
    res = client.update_cluster(req)
